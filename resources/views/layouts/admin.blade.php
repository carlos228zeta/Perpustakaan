...
<head>
    ...
    <title>@yield('title', 'Dashboard') - Library Management System</title>
...
<body>
        ...
        <aside class="app-sidebar" id="sidebar">
            <div class="sidebar-header">
                <a href="{{ route('dashboard') }}" class="brand-wrapper">
                    <img src="{{ asset('img/logo.png') }}" alt="Logo Cinta Kasih Tzu Chi" style="height: 36px; width: auto; object-fit: contain;">
                    <div>
                        <div class="brand-title" style="font-size: 0.95rem;">LMS Tzu Chi</div>
                        <div class="brand-subtitle" style="font-size: 0.7rem;">Cengkareng</div>
                    </div>
                </a>
            </div>
            ...
            <ul class="sidebar-menu">
                ...
                @if($role === 'admin')
                    <li class="sidebar-item {{ request()->is('admin/dashboard') ? 'active' : '' }}">
                        <a href="{{ route('admin.dashboard') }}"><i class="bi bi-speedometer2"></i> <span>Dashboard Admin</span></a>
                    </li>
                    <li class="sidebar-item {{ request()->is('admin/buku*') ? 'active' : '' }}">
                        <a href="{{ route('buku.index') }}"><i class="bi bi-journal-bookmark"></i> <span>Manajemen Buku</span></a>
                    </li>
                    <li class="sidebar-item {{ request()->is('admin/kategori*') ? 'active' : '' }}">
                        <a href="{{ route('kategori.index') }}"><i class="bi bi-tags"></i> <span>Kategori</span></a>
                    </li>
                    <li class="sidebar-item {{ request()->is('admin/siswa*') ? 'active' : '' }}">
                        <a href="{{ route('siswa.index') }}"><i class="bi bi-people"></i> <span>Data Siswa</span></a>
                    </li>
                    <li class="sidebar-item {{ request()->is('admin/guru*') ? 'active' : '' }}">
                        <a href="{{ route('guru.index') }}"><i class="bi bi-person-badge"></i> <span>Data Guru</span></a>
                    </li>
                    ...
                    <li class="sidebar-item {{ request()->is('admin/cetaklaporan*') ? 'active' : '' }}">
                        <a href="{{ url('admin/cetaklaporan') }}"><i class="bi bi-file-earmark-text"></i> <span>Laporan</span></a>
                    </li>
                    <li class="sidebar-item {{ request()->is('admin/pengaturan*') ? 'active' : '' }}">
                        <a href="{{ route('pengaturan.index') }}"><i class="bi bi-gear"></i> <span>Pengaturan Sistem</span></a>
                    </li>

                @elseif($role === 'librarian')
                    <li class="sidebar-item {{ request()->is('librarian/dashboard') ? 'active' : '' }}">
                        <a href="{{ route('librarian.dashboard') }}"><i class="bi bi-speedometer2"></i> <span>Dashboard Petugas</span></a>
                    </li>
                    <li class="sidebar-item {{ request()->is('admin/buku*') ? 'active' : '' }}">
                        <a href="{{ route('buku.index') }}"><i class="bi bi-journal-bookmark"></i> <span>Kelola Buku</span></a>
                    </li>
                    <li class="sidebar-item {{ request()->is('admin/kategori*') ? 'active' : '' }}">
                        <a href="{{ route('kategori.index') }}"><i class="bi bi-tags"></i> <span>Kategori</span></a>
                    </li>
                ...
                @elseif($role === 'teacher')
                    <li class="sidebar-item {{ request()->is('teacher/dashboard') ? 'active' : '' }}">
                        <a href="{{ route('teacher.dashboard') }}"><i class="bi bi-speedometer2"></i> <span>Dashboard Guru</span></a>
                    </li>
                    <li class="sidebar-item">
                        <a href="{{ route('public.books.index') }}"><i class="bi bi-search"></i> <span>Cari Buku</span></a>
                    </li>
                    <li class="sidebar-item {{ request()->is('profile*') ? 'active' : '' }}">
                        <a href="{{ route('profile.index') }}"><i class="bi bi-person"></i> <span>Profil Saya</span></a>
                    </li>

                @elseif($role === 'student')
                    <li class="sidebar-item {{ request()->is('student/dashboard') ? 'active' : '' }}">
                        <a href="{{ route('student.dashboard') }}"><i class="bi bi-speedometer2"></i> <span>Dashboard Siswa</span></a>
                    </li>
                    <li class="sidebar-item">
                        <a href="{{ route('public.books.index') }}"><i class="bi bi-search"></i> <span>Cari Buku</span></a>
                    </li>
            ...
            <div style="padding: 1rem 0.85rem; border-top: 1px solid var(--border-color);">
                <a href="{{ route('public.books.index') }}" class="btn-tzuchi btn-secondary-tzuchi" style="width: 100%; justify-content: flex-start;">
                    <i class="bi bi-globe"></i> <span>Website Publik</span>
                </a>
            </div>
        </aside>
        ...
        <div class="app-main">
            ...
            <header class="app-topbar">
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <button id="sidebar-toggle" style="background: none; border: none; font-size: 1.25rem; cursor: pointer; color: var(--text-main);">
                        <i class="bi bi-list"></i>
                    </button>
                    <h2 style="font-size: 1.1rem; margin: 0;">@yield('header_title', 'Dashboard')</h2>
                </div>
                ...
                <div style="display: flex; align-items: center; gap: 0.85rem;">
                    ...
                    <div style="position: relative;">
                        ...
                        <div id="notifDropdown" style="display: none; position: absolute; right: 0; top: 48px; width: 340px; background: var(--surface); border: 1px solid var(--border-color); border-radius: var(--radius-lg); box-shadow: var(--shadow-lg); z-index: 1200; overflow: hidden;">
                            <div style="padding: 0.85rem 1rem; border-bottom: 1px solid var(--border-color); background: var(--bg-color); font-weight: 600; font-size: 0.875rem; display: flex; justify-content: space-between; align-items: center;">
                                <span><i class="bi bi-clock-history" style="color: var(--primary);"></i> Pengingat Jatuh Tempo</span>
                                @if($reminderCount > 0)
                                    <span class="badge-tzuchi badge-danger" style="font-size: 0.7rem;">{{ $reminderCount }} Peminjaman</span>
                                @else
                                    <span class="badge-tzuchi badge-success" style="font-size: 0.7rem;">Tidak Ada</span>
                                @endif
                            </div>
                            ...
                            <div style="max-height: 300px; overflow-y: auto;">
                                @forelse($activeReminders as $item)
                                    ...
                                    @endphp
                                    <div style="padding: 0.85rem 1rem; border-bottom: 1px solid var(--border-color); font-size: 0.825rem;">
                                        @if($isOverdue)
                                        ...
                                        @endif
                                        <div style="color: var(--text-main); margin-top: 0.2rem; font-weight: 600;">{{ $item->book_title }}</div>
                                ...
                                @empty
                                    <div style="padding: 1.5rem 1rem; text-align: center; color: var(--text-muted); font-size: 0.85rem;">
                                        <i class="bi bi-check-circle-fill" style="font-size: 1.75rem; color: var(--primary); display: block; margin-bottom: 0.5rem;"></i>
                    ...
                    <div style="text-align: right; font-size: 0.85rem; margin-left: 0.5rem;">
                        <div style="font-weight: 600;">{{ auth()->user()->name }}</div>
                        <div style="color: var(--text-muted); text-transform: capitalize;">{{ auth()->user()->role->display_name ?? 'Pengguna' }}</div>
                    </div>

                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="btn-tzuchi btn-secondary-tzuchi btn-sm" title="Keluar">
            ...
            <main class="app-content">
                ...
                @if(session('error'))
                    <div style="background-color: #FEE2E2; color: var(--danger); border: 1px solid #FCA5A5; padding: 0.85rem 1rem; border-radius: var(--radius-md); margin-bottom: 1.25rem; font-size: 0.9rem;">
    ...
    <script>
        ...
        function updateAdminThemeIcon(theme) {
            if (theme === 'dark') {
                adminThemeIcon.className = 'bi bi-sun';
            } else {
                adminThemeIcon.className = 'bi bi-moon-stars';
            }
        }
